...
import { PrediccionGlobal } from '../models/PrediccionGlobal'
...
export class PrediccionService {
    ...
    /**
     * RF-15 — Valida que un partido este disponible para recibir apuestas.
     * Lanza error si el partido esta EN_CURSO, FINALIZADO, o faltan ≤ 15 min.
     */
    static validarApuestaAbierta(partido: { estado: string; fechaHora: Date }, minutosAntes: number = 15): void {
        if (partido.estado !== 'PROGRAMADO') {
            throw new Error(`Apuestas cerradas: el partido esta ${partido.estado}`)
        }
        ...
        if (msSinicio <= minutosAntes * 60_000) {
            ...
            throw new Error(
                mins > 0
                    ...
                    : 'Apuestas cerradas: el partido ya comenzo'
            )
        }
    }

    /** RF-04 — Crea una nueva prediccion de partido. */
    ...
    /** RF-04 — Obtiene una prediccion por id. */
    ...
    /** RF-04 / RF-07 — Evalua una prediccion contra el resultado real. */
    static evaluarPrediccion(
        ...
        if (!data) throw new Error(`Prediccion ${prediccionId} no encontrada.`)
    ...
    static calcularPuntajeUsuario(usuarioId: number, pollaId: number): number {
        const predicciones = Array.from(this.predicciones.values()).filter(
            (p) => p.usuarioId === usuarioId && p.pollaId === pollaId
        )

        const puntajePartidos = predicciones.reduce((sum, p) => sum + p.puntosObtenidos, 0)

        const prediccionGlobal = Array.from(this.prediccionesGlobales.values()).find(
            (pg) => pg.usuarioId === usuarioId && pg.pollaId === pollaId
        )
    ...
    /** RF-14 — Crea una prediccion global del torneo. */
    static crearPrediccionGlobal(usuarioId: number, pollaId: number): PrediccionGlobal {
    ...
    /** RF-14 — Obtiene la prediccion global de un usuario en una polla. */
    static obtenerPrediccionGlobal(usuarioId: number, pollaId: number): PrediccionGlobal | null {
        const data = Array.from(this.prediccionesGlobales.values()).find(
            (pg) => pg.usuarioId === usuarioId && pg.pollaId === pollaId
        )
        return data ? new PrediccionGlobal(data) : null
    }

    /** RF-14 — Actualiza la prediccion global de un usuario. */
