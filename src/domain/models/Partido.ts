...
export class Partido implements IPartido {
    ...
    /**
     * RF-15 — Verifica si las apuestas estan abiertas para este partido.
     * Cierra automaticamente N minutos antes del inicio (default: 15 min).
     */
    estaDisponibleParaApuesta(minutosAntes: number = 15): boolean {
        if (this.estado !== EstadoPartido.PROGRAMADO) return false
        const msSinicio = this.fechaHora.getTime() - new Date().getTime()
        return msSinicio > minutosAntes * 60_000
    }

    /**
     * RF-15 — Registra o actualiza el resultado del partido.
     * Permite editar desde EN_CURSO o FINALIZADO (correccion de errores).
     */
    actualizarResultado(golesLocal: number, golesVisitante: number): void {
        if (this.estado !== EstadoPartido.EN_CURSO && this.estado !== EstadoPartido.FINALIZADO) {
