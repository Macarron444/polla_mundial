...
export class Usuario implements IUsuario {
     ...
     * El hash de contrasena se genera en usuario.service.ts (bcrypt).
     */
    static prepararRegistro(email: string, nombre: string): Omit<IUsuario, 'id' | 'passwordHash'> {
        return {
            email,
            nombre,
    ...
    /** RF-01 — Cierra la sesion (el token JWT se invalida en el middleware). */
    cerrarSesion(): void {
        // La logica de invalidacion del token se delega a usuario.middleware.ts
    }

    /**
     * RF-09 — Configura que canales de notificacion tiene activos el usuario.
     * push  → Service Worker + VAPID (requiere consentimiento explicito)
