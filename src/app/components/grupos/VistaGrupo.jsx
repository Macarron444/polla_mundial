...
import { agregarMiembro, cambiarRol, eliminarMiembro, eliminarGrupo, getRolEnGrupo } from '../../../core/storage/grupos.js'
...
function VistaGrupo({ grupo, usuario, partidos, equipos, onVolver, onCambio }) {
    ...
    const solicitudesPendientes = esAdmin ? getSolicitudesPorGrupo(grupo.id).length : 0
    ...
    const handleCambiarRol = (uid, rol) => {
        setError('')
        try { cambiarRol(grupo.id, uid, rol); onCambio() }
        catch (e) { setError(e.message) }
    }
    const handleEliminarMiembro = (uid) => {
        setError('')
        try { eliminarMiembro(grupo.id, uid); setConfirmarDel(null); onCambio() }
        catch (e) { setError(e.message) }
    }
    ...
    return (
        <div>
            ...
            {error && <div style={{ background: '#2a0d0d', border: '1px solid #c92a2a44', color: '#ff8787', fontSize: 11, padding: '8px 14px', borderRadius: 8, marginBottom: 14 }}>⚠️ {error}</div>}
                ...
                {tabs.map((t) => (
                    <button key={t} onClick={() => setTabActiva(t)} style={{
                        ...
                        {t === 'Miembros' && solicitudesPendientes > 0 && (
                            <span style={{ position: 'absolute', top: -4, right: -4, background: '#ffd43b', color: '#000', fontSize: 9, fontWeight: 800, borderRadius: '50%', width: 16, height: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {tabActiva === 'Miembros' && esAdmin && (
                <div>
                    ...
                    <div style={{ background: '#0d1628', border: '1px solid #1e2a45', borderRadius: 12, padding: '16px 18px', marginBottom: 20 }}>
                        ...
                        <input className="login-input" placeholder="Busca por nombre o correo..."
                            value={busqueda} onChange={(e) => setBusqueda(e.target.value)} />
                        {resultadosBusqueda.map((u) => (
                            <div key={u.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#060c18', border: '1px solid #1e2a45', borderRadius: 8, padding: '8px 12px', marginTop: 6 }}>
                                <div>
                    ...
                    <div style={{ fontSize: 9, fontWeight: 700, color: '#4a6fa5', letterSpacing: '0.07em', marginBottom: 10 }}>MIEMBROS ({grupo.miembros.length})</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {grupo.miembros.map((m) => {
                            const esMiSelf = m.usuarioId === usuario.id
                            const esElCreador = m.usuarioId === grupo.creadoPor
                            return (
                                <div key={m.usuarioId} style={{ background: '#0d1628', border: `1px solid ${esMiSelf ? '#3b5bdb55' : '#1e2a45'}`, borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1 }}>
                                        <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#1e2a45', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>
                                            {esElCreador ? '👑' : m.rol === 'ADMIN' ? '🛡️' : '👤'}
                                        </div>
                                        <div>
                                            <div style={{ fontSize: 13, fontWeight: 700, color: '#e2e8f0' }}>{m.nombre} {esMiSelf && <span style={{ fontSize: 10, color: '#748ffc' }}>(tú)</span>}</div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        ...
                                        {esAdmin && !esMiSelf && (
                                            <select value={m.rol} onChange={(e) => handleCambiarRol(m.usuarioId, e.target.value)}
                                                style={{ background: '#060c18', border: '1px solid #1e2a45', color: '#748ffc', fontSize: 10, fontWeight: 700, padding: '4px 8px', borderRadius: 6, fontFamily: 'inherit', cursor: 'pointer' }}>
