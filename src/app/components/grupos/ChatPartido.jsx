                        ...
                        <div key={c.id} style={{
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                                ...
                                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                    <span style={{ fontSize: 9, color: '#94a3b8' }}>{fecha}</span>
                                    {esMio && (
                                        <button onClick={() => handleEliminar(c.id)}
                                            style={{ background: 'none', border: 'none', color: '#dc2626', cursor: 'pointer', fontSize: 12, padding: 0, fontFamily: 'inherit' }}>
                                            ✕
                                        </button>
                                    )}
                                </div>
                            </div>
                            <div style={{ fontSize: 12, color: '#1e293b', lineHeight: 1.5 }}>{c.texto}</div>
                        </div>
                    )
                })}
            </div>
