...
import { getPredicionesPorUsuario } from '../../core/storage/prediccionesGrupo.js'
...
function TabRanking({ usuario }) {
    ...
    const cargarRankingGlobal = async () => {
        ...
        try {
            ...
            // Para cada usuario sumar sus predicciones PROPIAS (independiente de grupos)
            const filas = await Promise.all(usuarios.map(async (u) => {
                let pts = 0, exactas = 0, correctas = 0, fallidas = 0, pendientes = 0
                try {
                    // getPredicionesPorUsuario busca TODAS las predicciones del usuario
                    // en todos los grupos — pero los puntos del grupo no afectan esto
                    // porque aquí contamos solo predicciones con estado resuelto
                    const preds = await getPredicionesPorUsuario(u.id)
                    preds.forEach((p) => {
                        // Solo contar predicciones del usuario, no del grupo
                        pts       += p.pts ?? 0
                ...
                return {
                    ...
                    nombre: u.nombre,
                    email: u.email,
                    pts, exactas, correctas, fallidas, pendientes,
                    esMio: u.id === usuario.id,
                }
            }))

            const sorted = filas.sort((a, b) =>
                b.pts - a.pts || b.exactas - a.exactas || b.correctas - a.correctas
            )
            setRanking(sorted)
        } catch (e) {
        ...
        <div className="tab-section">
                    ...
                    return (
                        <div key={r.id} style={{
                            ...
                            <div style={{ textAlign: 'right', flexShrink: 0 }}>
                                <div style={{ fontSize: 22, fontWeight: 800, color: pos === 1 ? '#ffd43b' : '#e2e8f0' }}>
                                    {r.pts}
                                </div>
            ...
            <div style={{ marginTop: 20, padding: '12px 16px', background: '#0d1628', border: '1px solid #1e2a45', borderRadius: 10 }}>
                <div style={{ fontSize: 10, color: '#4a6fa5', lineHeight: 1.6 }}>
                    ℹ️ El ranking global muestra los puntos acumulados de cada jugador en <strong style={{ color: '#748ffc' }}>todas sus predicciones individuales</strong>, independiente de los grupos a los que pertenece. Los rankings de cada grupo son independientes y no afectan este ranking.
                </div>
            </div>
        </div>
    )
