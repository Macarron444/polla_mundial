...
function TabPredicciones({ usuario, partidos, equipos }) {
    ...
    const agregarPrediccion = () => {
        ...
        if (!partido || getBetStatus(partido) !== 'ABIERTA') {
            alert('Este partido ya no acepta apuestas')
            return
        }
        setPreds((prev) => [
            ...prev,
    ...
    const savePred = (partidoId) => {
        ...
        if (!partido || getBetStatus(partido) !== 'ABIERTA') {
            alert('Las apuestas para este partido ya estan cerradas.')
            setEditId(null)
            return
        }
        setPreds((prev) =>
    ...
    return (
        <div className="tab-section">
            <div className="stat-cards">
                ...
                <StatCard
                    label="EXACTAS"
                    value={preds.filter((p) => p.estado === 'EXACTA').length}
                    accent="#69db7c"
                />
                <StatCard
                    label="CORRECTAS"
                    value={preds.filter((p) => p.estado === 'CORRECTA').length}
                    accent="#a9e34b"
                />
                <StatCard
                    label="FALLIDAS"
                    value={preds.filter((p) => p.estado === 'FALLIDA').length}
                    accent="#ff8787"
                />
            </div>
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1, minWidth: 200 }}>
                                ...
                                <select
                                    value={newPartidoId}
                                    onChange={(e) => setNewPartidoId(e.target.value)}
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                    <input
                                        ...
                                        min="0"
                                        max="20"
                                        style={{ ...scoreInputStyle, width: 38, fontSize: 16 }}
                                    />
                                    <span style={{ color: '#4a6fa5', fontWeight: 800, fontSize: 16 }}>–</span>
                                    <input
                                        value={newGv}
                                        onChange={(e) => setNewGv(e.target.value)}
                                        type="number"
                                        min="0"
                                        max="20"
                                        style={{ ...scoreInputStyle, width: 38, fontSize: 16 }}
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                                <button onClick={agregarPrediccion} style={btnStyle('#2f9e44')}>
                                    ✓ Guardar
                                </button>
                                <button
                                    onClick={() => {
                    ...
                    return (
                        ...
                        >
                                ...
                                <div className={`pred-banner pred-banner--${finalizado ? 'finalizado' : 'bloqueado'}`}>
                                    ...
                                    <span
                                        style={{
                                            fontSize: 11,
                                            fontWeight: 700,
                                            color: finalizado ? '#69db7c' : '#ffa94d',
                                            letterSpacing: '0.05em',
                                        }}
                                    >
                                        {finalizado
                                            ...
                                            : bloqueadoProximo
                                                ? `APUESTAS CIERRAN EN ${minsLeft} MIN · El partido esta por comenzar`
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                                    ...
                                    {isEdit ? (
                                        <>
                                            ...
                                            <input
                                                ...
                                                style={scoreInputStyle}
                                            />
                                            <button onClick={() => savePred(partidoId)} style={btnStyle('#2f9e44')}>
                                                ✓
                                            </button>
                                        ...
                                        <>
                                            <span style={{ fontSize: 16, fontWeight: 800, color: c.text, letterSpacing: 3 }}>
                                                {golesL} – {golesV}
                                            </span>
                                            {editable && (
                                                <button
                                                    ...
                                                    style={btnStyle('#4a6fa5')}
                                                >
                                                    ✎
                                                </button>
                                            )}
                                            {!editable && !finalizado && (
                                                <span style={{ fontSize: 10, color: '#6b7280', fontStyle: 'italic' }}>🔒</span>
                                            )}
                                        </>
                                    )}
                                </div>
                                ...
                                <span style={{ fontSize: 18, fontWeight: 800, color: pts > 0 ? '#ffd43b' : '#4a6fa5', minWidth: 40, textAlign: 'right' }}>
                                    +{pts}pts
                                </span>
                            </div>
                        </div>
            ...
            <div className="pred-global">
                ...
                >
                    ⚽ Prediccion Global del Torneo (RF-14)
                </div>
                <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                    {[
                        { label: 'Campeon', value: '🇦🇷 Argentina' },
                        { label: 'Subcampeon', value: '🇫🇷 Francia' },
                        { label: 'Max. Goleador', value: 'L. Martinez' },
                    ].map((item) => (
                        <div
                            key={item.label}
                            style={{
                                background: '#12180a',
                                border: '1px solid #ffd43b22',
                                borderRadius: 8,
                                padding: '10px 16px',
                            }}
                        >
