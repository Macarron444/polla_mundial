...
// ── HELPER: comparación segura de IDs (evita Number vs String) ────────────────
...
export async function agregarMiembro(grupoId, usuario, rol = 'PARTICIPANTE') {
    ...
    const grupo  = grupos.find((g) => mismoId(g.id, grupoId))
    if (!grupo) throw new Error('Grupo no encontrado')
    if (grupo.miembros.some((m) => mismoId(m.usuarioId, usuario.id)))
...
export async function cambiarRol(grupoId, usuarioId, nuevoRol) {
    ...
    const miembro = grupo.miembros.find((m) => mismoId(m.usuarioId, usuarioId))
...
export async function eliminarMiembro(grupoId, usuarioId) {
    ...
    grupo.miembros = grupo.miembros.filter((m) => !mismoId(m.usuarioId, usuarioId))
    await saveGrupo(grupo)
    return grupo
}
...
export async function eliminarGrupo(grupoId, usuarioId) {
    ...
    const grupo  = grupos.find((g) => mismoId(g.id, grupoId))
    if (!grupo) throw new Error('Grupo no encontrado')
    if (!mismoId(grupo.creadoPor, usuarioId)) throw new Error('Solo el creador puede eliminar el grupo')
...
export async function actualizarConfigGrupo(grupoId, usuarioId, cambios) {
    ...
    const grupo  = grupos.find((g) => mismoId(g.id, grupoId))
    if (!grupo) throw new Error('Grupo no encontrado')
    if (!mismoId(grupo.creadoPor, usuarioId)) throw new Error('Solo el creador puede editar la configuración')
...
export async function getGruposDeUsuario(usuarioId) {
    const grupos = await getGrupos()
    // mismoId evita que Number vs String cause que el usuario no vea sus grupos
    return grupos.filter((g) => g.miembros.some((m) => mismoId(m.usuarioId, usuarioId)))
}
...
// getRolEnGrupo también usa mismoId para ser consistente
export function getRolEnGrupo(grupo, usuarioId) {
    return grupo.miembros.find((m) => mismoId(m.usuarioId, usuarioId))?.rol ?? null
}
