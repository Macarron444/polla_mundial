...
export class Polla implements IPolla {
  ...
  /**
   * RF-12 — Obtiene las reglas de puntuación por defecto.
   */
  ...
  /**
   * RF-12 — Actualiza las reglas de puntuación si no están bloqueadas.
   */
  actualizarReglas(reglas: Partial<IReglaPuntuacion>): void {
    if (this.reglasBlockeadas) {
      throw new Error("Las reglas de puntuación están bloqueadas");
    }
    this.reglasPuntuacion = { ...this.reglasPuntuacion, ...reglas };
  }

  /**
   * RF-12 — Bloquea las reglas de puntuación.
   */
  bloquearReglas(): void {
  ...
  /**
   * RF-02 — Obtiene las reglas de puntuación.
   */
  ...
  /**
   * RF-10 — Cierra la polla (no se pueden hacer más predicciones).
   */
  cerrar(): void {
    if (this.estado === EstadoPolla.CERRADA) {
      throw new Error("La polla ya está cerrada");
    }
  ...
  /**
   * RF-03 — Verifica si la polla es pública.
   */
  esPublica(): boolean {
