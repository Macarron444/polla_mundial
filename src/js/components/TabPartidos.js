function TabPartidos({ partidos, setPartidos }) {
  const [editId, setEditId] = useState(null);
  const [gl, setGl] = useState("");
  const [gv, setGv] = useState("");

  const faseLabel = {
    GRUPOS:"Fase de Grupos", OCTAVOS:"Octavos de Final",
    CUARTOS:"Cuartos", SEMIFINAL:"Semifinal", FINAL:"Final",
  };

  // [TS: Partido.avanzarEstado() → GestorPartidos.cambiarEstado()]
  const avanzarEstado = (p) => {
    ...
    setPartidos(prev => prev.map(x =>
      x.id === p.id ? { ...x, estado: ciclo[idx + 1] } : x
    ));
  };

  // [TS: Partido.actualizarResultado() → GestorPartidos.editarResultado()]
  const saveResult = (p) => {
    ...
    setPartidos(prev => prev.map(x =>
      x.id === p.id ? { ...x, golesL: l, golesV: v } : x
    ));
