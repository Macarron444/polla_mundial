function TabPredicciones({ partidos }) {
  ...
  const partidosDisponibles = partidos.filter(p =>
    getBetStatus(p) === "ABIERTA" && !preds.some(pr => pr.partidoId === p.id)
  );

  // [TS: PrediccionService.validarApuestaAbierta() + crearPrediccion()]
  const agregarPrediccion = () => {
    const id = parseInt(newPartidoId);
    ...
    setPreds(prev => [...prev, { partidoId: id, golesL: parseInt(newGl)||0, golesV: parseInt(newGv)||0, estado:"PENDIENTE", pts:0 }]);
    setNewPartidoId(""); setNewGl("0"); setNewGv("0"); setShowAddForm(false);
  };

  // [TS: PrediccionService.validarApuestaAbierta() antes de actualizar]
  const savePred = (partidoId) => {
    ...
    setPreds(prev => prev.map(p => p.partidoId===partidoId
      ? { ...p, golesL:parseInt(gl)||0, golesV:parseInt(gv)||0 } : p
    ));
    setEditId(null);
  };
  ...
  const scoreInputStyle = {
    ...
    color:"#e2e8f0", borderRadius:5, padding:"3px", fontSize:14, fontWeight:700, fontFamily:"inherit",
  };
  ...
  return (
    <div className="tab-section">
      ...
      {/* Botón / Formulario agregar predicción */}
      <div style={{ marginBottom:14 }}>
        {!showAddForm ? (
          <button onClick={() => setShowAddForm(true)} disabled={partidosDisponibles.length === 0} style={{
            ...
            cursor: partidosDisponibles.length===0 ? "not-allowed" : "pointer",
          ...
          <div style={{ background:"#0d1628", border:"1px solid #3b5bdb44", borderRadius:10, padding:"16px 18px" }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#748ffc", marginBottom:12, letterSpacing:"0.05em" }}>⚽ Nueva Predicción</div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:4, flex:1, minWidth:200 }}>
                ...
                <select value={newPartidoId} onChange={e => setNewPartidoId(e.target.value)} className="form-control--dark">
                  ...
                  {partidosDisponibles.map(p => {
                    ...
                    return <option key={p.id} value={p.id}>{loc.flag} {loc.nombre} vs {vis.nombre} {vis.flag} · {p.fecha}</option>;
                  })}
                </select>
              </div>
              <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                <label style={{ fontSize:9, color:"#4a6fa5", fontWeight:700, letterSpacing:"0.07em" }}>MI PREDICCIÓN</label>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {preds.map(({ partido: _p, golesL, golesV, estado, pts, partidoId }) => {
          const partido = partidos.find(p=>p.id===partidoId);
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  ...
                  {isEdit ? (
                    ...
                    <>
                      ...
                      {editable && <button onClick={()=>{setEditId(partidoId);setGl(golesL);setGv(golesV);}} style={btnStyle("#4a6fa5")}>✎</button>}
      ...
      {/* Predicción global */}
      <div className="pred-global">
        <div style={{ fontSize:12, fontWeight:700, color:"#ffd43b", marginBottom:10, letterSpacing:"0.05em" }}>⚽ Predicción Global del Torneo (RF-14)</div>
        <div style={{ display:"flex", gap:20, flexWrap:"wrap" }}>
          {[{ label:"Campeón", value:"🇦🇷 Argentina" }, { label:"Subcampeón", value:"🇫🇷 Francia" }, { label:"Máx. Goleador", value:"L. Martínez" }].map(item => (
            <div key={item.label} style={{ background:"#12180a", border:"1px solid #ffd43b22", borderRadius:8, padding:"10px 16px" }}>
